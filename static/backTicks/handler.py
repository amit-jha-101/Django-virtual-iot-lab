...
def handler(event, context):
	...
	event['latency']:(Decimal(event['time1'])-Decimal(time.time()))
...
#Rules will execute here
	...
	if(valid):
		...
		table = client.Table('backTicks') #Sensor Name
		table.put_item(Item= {'SensorName':'backTicks','timeStamp': str(datetime.datetime.now()),'value': event,'valid':True})
